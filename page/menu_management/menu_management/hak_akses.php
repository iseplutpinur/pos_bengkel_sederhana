...
if (isset($_GET['id'])) {
    ...
    $menu_title = query("SELECT `menu_title` FROM `tb_user_menu` WHERE `id_menu`='$id_menu'");
    ...
    for ($i = 0; $i < count($user_level); $i++) {
        ...
        $result = query("SELECT 'id_menu' FROM tb_user_menu_access WHERE tb_user_menu_access.id_menu = '$id_menu' AND tb_user_menu_access.id_level = '$user_level_id'");
...
<div class="container-fluid">
    <div class="card card-primary card-outline">
        <div class="card-header">
            <h3 class="card-title">Hak Akses Menu <b><?= $menu_title[0]['menu_title']; ?></b></h3>
        </div> <!-- /.card-body -->
        <div class="card-body">
                        ...
                        foreach ($user_level as $menu) : ?>
                            <tr>
                                ...
                                <td style="text-align:center;">
                                    <label class="switch">
                                        <?php if ($menu['akses']) : ?>
                                            <input type="checkbox" class="primary" checked="" onchange="gantiHakAkses('<?= $menu['id_level']; ?>', '<?= $id_menu; ?>', 1)">
                                        <?php else : ?>
                                            <input type="checkbox" class="primary" onchange="gantiHakAkses('<?= $menu['id_level']; ?>', '<?= $id_menu; ?>', 0)">
                                        <?php endif; ?>
